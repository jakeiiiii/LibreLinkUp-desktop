...
- UI chrome uses a neutral slate-gray accent (no red): `--accent` (fills — header bar, Log button, login button, beep OK, checkbox), `--accent-hover` / `--accent-pressed` (button states + header select), `--accent-fg` (text accents — login/modal headings, gear & menu hovers), `--focus` (input focus border). Light mode `--accent: #4a4f57`; dark mode lightens fills to `#5a6068` and text accents to `#b8bdc5` for contrast. Only the glucose severity colors (`--green`, `--yellow`, `--dark-red`/`--glucose-high-severe`) remain colored, since they encode data.
