...
- Only the chrome is neutralized; the glucose alert colors are the originals, untouched. `glucoseColor()` returns red `#cc0000` (<4), neutral `--glucose-normal` (4–10), amber `#cc8800` (10.1–14.9), and `--glucose-high-severe` (`#6b1010` light / `#ff6b6b` dark, ≥15). The chart keeps its orange high / red low threshold dashes (in both the Chart.js annotation config and the custom `beforeDraw` plugin) and its red current-reading dot (`rgba(200,50,50,0.9)`). The unused `--green`/`--yellow`/`--dark-red` vars were removed.
