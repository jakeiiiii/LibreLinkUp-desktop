...
- Fully red-free, neutral palette. UI chrome uses a slate-gray accent: `--accent` (fills — header bar, Log button, login button, beep OK, checkbox), `--accent-hover` / `--accent-pressed` (button states + header select), `--accent-fg` (text accents — login/modal headings, gear & menu hovers), `--focus` (input focus border). Light mode `--accent: #4a4f57`; dark mode lightens fills to `#5a6068` and text accents to `#b8bdc5` for contrast.
- Only glucose *alerts* stay colored; everything non-alert is neutral. `glucoseColor()` returns red `#cc0000` (<4), neutral `--glucose-normal` (4–10), amber `#cc8800` (10.1–14.9), and `--glucose-high-severe` (`#6b1010` light / `#ff6b6b` dark, ≥15). The chart's high/low threshold lines keep their orange/red dashes (in both the Chart.js annotation config and the custom `beforeDraw` plugin). The current-reading dot is neutral — it uses `--chart-line` (kept in sync by `applyTheme()` via `datasets[1].backgroundColor`), since it's a position marker, not a per-value alert. The unused `--green`/`--yellow`/`--dark-red` vars were removed.
